...
class RabbitMQBus extends Bus {
    ...
    publish(message) {
        ...
        if (message.hasReturnCallback) {
            ...
            this._onErrorCalbacks.set(key, message.onReturn);

            setTimeout(() => {
                this._onErrorCalbacks.delete(key);
            }, 30000);
        ...
        try {
            if (message.isToQueue) {
                ...
                this._channel.publish(message.exchange, message.routingKey, message.content, message.options, (error) => {
                    if (error) {
                        this._logger.warn('Message will be re-queued by not receiving ack', error);
                        this._buffer.push(message);
                    }
    ...
    registerEvenHandlers() {
        ...
        this._channel.on('return', (msg) => {
            ...
            if (msg.properties.headers.hasOwnProperty('return') &&
                this._onErrorCalbacks.has(msg.properties.headers.return)) {

                const isToRequeue = this._onErrorCalbacks.get(msg.properties.headers.return)(msg);
